...
### make works, check build scripts
$works = @()
...
$pool = [System.Collections.Generic.List[object]]@()
$iWork = 0
$abort = ''
$failures = @()
...
try {
	for() {
		### add works to pool, BeginInvoke
		while($pool.Count -lt $MaximumBuilds -and $iWork -lt $works.Count) {
			$work = $works[$iWork++]
			$pool.Add($work)
		...
		foreach($work in $pool) {$waits.Add($work.Job.AsyncWaitHandle)}
	...
	foreach($work in $works) {
		...
		$_ = if ($r = $work.Build.Result['Value']) {
			...
			"'$($work.Build.File)' invocation failed: $reason"
		}
		if (!$_) {
			$_ = $reason
		}
